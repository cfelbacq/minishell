...
/*   Updated: 2016/04/11 18:29:18 by cfelbacq         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_list			*epur_list(t_list *start)
{
	...
	{
		if (ft_strcmp(tmp->content, ".") == 0)
			tmp = free_if_dot(previous, tmp, new);
		if (ft_strcmp(tmp->content, "..") == 0)
		{
			if (ft_strcmp(previous->content, "/") == 0)
				return (free_dot_left(tmp, previous, pre_previous, new));
			else if (pre_previous != NULL)
				tmp = free_if_dot_dot(pre_previous, previous, tmp, new);
		}
